...
export const getProfilDusun = cache(async (): Promise<ProfilDusun | null> => {
  const { rows } = await query<ProfilDusun>(
    `SELECT id, nama_dusun, nama_desa, nama_kecamatan, nama_kabupaten,
            nama_provinsi, kode_pos, nama_kepala_dusun, foto_kepala_dusun,
            pesan_sambutan, sejarah_dusun, visi, misi, email, no_telepon,
            alamat_kantor, link_facebook, link_instagram, link_youtube,
